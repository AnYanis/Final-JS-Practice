...
// Return the ETA as a native Date object (same weekend-skipping logic)
export function calculateDeliveryEtaDate(deliveryOption) {
    const daysToAdd = Number(deliveryOption?.deliveryDays ?? 0);
    let date = dayjs();
    let remaining = isNaN(daysToAdd) ? 0 : daysToAdd;
    while (remaining > 0) {
    ...
    return date.toDate(); // Removed formatting
}